...
import Features_2_0 as features
...
def logistic_regression():
    ...
    plt.ylabel('True')
    plt.show()
    plt.savefig('plots/confusion_matrix_features_2_0_2004-2016.png')
...
if __name__ == "__main__":
    ...
    print("Predicted classes for a game:", predicted_classes)

    # for the same game, but from the perspective of the opponent

    new_data = np.array(
        [-38, 0, 27.4, 99, 170, 2, 4, 12, 6, 19, 36, 8, 25, 22, 10, 32.9, 137, 175, 4, 2, 6, 12, 25, 22, 10, 19,
         36, 8])
    new_data = scaler.fit_transform(np.reshape(new_data, (1, -1)))
    # predicted_classes = loaded_model.predict(np.reshape(new_data, (1, -1)))
    predicted_classes = loaded_model.predict(new_data)
